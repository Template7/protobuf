// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.34.1
// 	protoc        v5.26.1
// source: proto/template7/user/v1.proto

package v1

import (
	auth "github.com/Template7/protobuf/gen/proto/template7/auth"
	wallet "github.com/Template7/protobuf/gen/proto/template7/wallet"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type GetUserWalletResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Wallets []*wallet.Wallet `protobuf:"bytes,1,rep,name=wallets,proto3" json:"wallets,omitempty"`
}

func (x *GetUserWalletResponse) Reset() {
	*x = GetUserWalletResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_proto_template7_user_v1_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *GetUserWalletResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUserWalletResponse) ProtoMessage() {}

func (x *GetUserWalletResponse) ProtoReflect() protoreflect.Message {
	mi := &file_proto_template7_user_v1_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUserWalletResponse.ProtoReflect.Descriptor instead.
func (*GetUserWalletResponse) Descriptor() ([]byte, []int) {
	return file_proto_template7_user_v1_proto_rawDescGZIP(), []int{0}
}

func (x *GetUserWalletResponse) GetWallets() []*wallet.Wallet {
	if x != nil {
		return x.Wallets
	}
	return nil
}

type UserInfoResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	UserId   string             `protobuf:"bytes,1,opt,name=userId,proto3" json:"userId,omitempty"`
	Role     auth.Role          `protobuf:"varint,2,opt,name=role,proto3,enum=auth.Role" json:"role,omitempty"`
	Status   auth.AccountStatus `protobuf:"varint,3,opt,name=status,proto3,enum=auth.AccountStatus" json:"status,omitempty"`
	Nickname string             `protobuf:"bytes,4,opt,name=nickname,proto3" json:"nickname,omitempty"`
	Email    string             `protobuf:"bytes,5,opt,name=email,proto3" json:"email,omitempty"`
}

func (x *UserInfoResponse) Reset() {
	*x = UserInfoResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_proto_template7_user_v1_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UserInfoResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UserInfoResponse) ProtoMessage() {}

func (x *UserInfoResponse) ProtoReflect() protoreflect.Message {
	mi := &file_proto_template7_user_v1_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UserInfoResponse.ProtoReflect.Descriptor instead.
func (*UserInfoResponse) Descriptor() ([]byte, []int) {
	return file_proto_template7_user_v1_proto_rawDescGZIP(), []int{1}
}

func (x *UserInfoResponse) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *UserInfoResponse) GetRole() auth.Role {
	if x != nil {
		return x.Role
	}
	return auth.Role(0)
}

func (x *UserInfoResponse) GetStatus() auth.AccountStatus {
	if x != nil {
		return x.Status
	}
	return auth.AccountStatus(0)
}

func (x *UserInfoResponse) GetNickname() string {
	if x != nil {
		return x.Nickname
	}
	return ""
}

func (x *UserInfoResponse) GetEmail() string {
	if x != nil {
		return x.Email
	}
	return ""
}

type UpdateUserInfoRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Nickname string `protobuf:"bytes,1,opt,name=nickname,proto3" json:"nickname,omitempty"`
}

func (x *UpdateUserInfoRequest) Reset() {
	*x = UpdateUserInfoRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_proto_template7_user_v1_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateUserInfoRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateUserInfoRequest) ProtoMessage() {}

func (x *UpdateUserInfoRequest) ProtoReflect() protoreflect.Message {
	mi := &file_proto_template7_user_v1_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateUserInfoRequest.ProtoReflect.Descriptor instead.
func (*UpdateUserInfoRequest) Descriptor() ([]byte, []int) {
	return file_proto_template7_user_v1_proto_rawDescGZIP(), []int{2}
}

func (x *UpdateUserInfoRequest) GetNickname() string {
	if x != nil {
		return x.Nickname
	}
	return ""
}

type CreateUserRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Username string    `protobuf:"bytes,1,opt,name=username,proto3" json:"username,omitempty"`
	Password string    `protobuf:"bytes,2,opt,name=password,proto3" json:"password,omitempty"`
	Role     auth.Role `protobuf:"varint,3,opt,name=role,proto3,enum=auth.Role" json:"role,omitempty"`
	Nickname string    `protobuf:"bytes,4,opt,name=nickname,proto3" json:"nickname,omitempty"`
	Email    string    `protobuf:"bytes,5,opt,name=email,proto3" json:"email,omitempty"`
}

func (x *CreateUserRequest) Reset() {
	*x = CreateUserRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_proto_template7_user_v1_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateUserRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateUserRequest) ProtoMessage() {}

func (x *CreateUserRequest) ProtoReflect() protoreflect.Message {
	mi := &file_proto_template7_user_v1_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateUserRequest.ProtoReflect.Descriptor instead.
func (*CreateUserRequest) Descriptor() ([]byte, []int) {
	return file_proto_template7_user_v1_proto_rawDescGZIP(), []int{3}
}

func (x *CreateUserRequest) GetUsername() string {
	if x != nil {
		return x.Username
	}
	return ""
}

func (x *CreateUserRequest) GetPassword() string {
	if x != nil {
		return x.Password
	}
	return ""
}

func (x *CreateUserRequest) GetRole() auth.Role {
	if x != nil {
		return x.Role
	}
	return auth.Role(0)
}

func (x *CreateUserRequest) GetNickname() string {
	if x != nil {
		return x.Nickname
	}
	return ""
}

func (x *CreateUserRequest) GetEmail() string {
	if x != nil {
		return x.Email
	}
	return ""
}

var File_proto_template7_user_v1_proto protoreflect.FileDescriptor

var file_proto_template7_user_v1_proto_rawDesc = []byte{
	,
}

var (
	file_proto_template7_user_v1_proto_rawDescOnce sync.Once
	file_proto_template7_user_v1_proto_rawDescData = file_proto_template7_user_v1_proto_rawDesc
)

func file_proto_template7_user_v1_proto_rawDescGZIP() []byte {
	file_proto_template7_user_v1_proto_rawDescOnce.Do(func() {
		file_proto_template7_user_v1_proto_rawDescData = protoimpl.X.CompressGZIP(file_proto_template7_user_v1_proto_rawDescData)
	})
	return file_proto_template7_user_v1_proto_rawDescData
}

var file_proto_template7_user_v1_proto_msgTypes = make([]protoimpl.MessageInfo, 4)
var file_proto_template7_user_v1_proto_goTypes = []interface{}{
	(*GetUserWalletResponse)(nil), // 0: user.getUserWalletResponse
	(*UserInfoResponse)(nil),      // 1: user.userInfoResponse
	(*UpdateUserInfoRequest)(nil), // 2: user.updateUserInfoRequest
	(*CreateUserRequest)(nil),     // 3: user.createUserRequest
	(*wallet.Wallet)(nil),         // 4: wallet.wallet
	(auth.Role)(0),                // 5: auth.role
	(auth.AccountStatus)(0),       // 6: auth.accountStatus
}
var file_proto_template7_user_v1_proto_depIdxs = []int32{
	4, // 0: user.getUserWalletResponse.wallets:type_name -> wallet.wallet
	5, // 1: user.userInfoResponse.role:type_name -> auth.role
	6, // 2: user.userInfoResponse.status:type_name -> auth.accountStatus
	5, // 3: user.createUserRequest.role:type_name -> auth.role
	4, // [4:4] is the sub-list for method output_type
	4, // [4:4] is the sub-list for method input_type
	4, // [4:4] is the sub-list for extension type_name
	4, // [4:4] is the sub-list for extension extendee
	0, // [0:4] is the sub-list for field type_name
}

func init() { file_proto_template7_user_v1_proto_init() }
func file_proto_template7_user_v1_proto_init() {
	if File_proto_template7_user_v1_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_proto_template7_user_v1_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*GetUserWalletResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_proto_template7_user_v1_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UserInfoResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_proto_template7_user_v1_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UpdateUserInfoRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_proto_template7_user_v1_proto_msgTypes[3].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateUserRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_proto_template7_user_v1_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   4,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_proto_template7_user_v1_proto_goTypes,
		DependencyIndexes: file_proto_template7_user_v1_proto_depIdxs,
		MessageInfos:      file_proto_template7_user_v1_proto_msgTypes,
	}.Build()
	File_proto_template7_user_v1_proto = out.File
	file_proto_template7_user_v1_proto_rawDesc = nil
	file_proto_template7_user_v1_proto_goTypes = nil
	file_proto_template7_user_v1_proto_depIdxs = nil
}
